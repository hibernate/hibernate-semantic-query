...
public abstract class PathResolverStandardTemplate extends AbstractPathResolverImpl {
	...
	public PathResolverStandardTemplate(ResolutionContext resolutionContext) {
	...
	public Binding resolvePath(HqlParser.DotIdentifierSequenceContext path, EntityType subclassIndicator) {
		...
		if ( pathText.contains( "." ) ) {
			final FromElement identifiedFromElement = resolutionContext().getFromElementLocator().findFromElementByIdentificationVariable( rootPart );
			if ( identifiedFromElement != null ) {
				validatePathRoot( identifiedFromElement );
				final AttributeBindingSource terminalLhs = resolveAnyIntermediateAttributePathJoins( identifiedFromElement, parts, 1 );
		...
		final FromElement root = resolutionContext().getFromElementLocator().findFromElementExposingAttribute( rootPart );
		if ( root != null ) {
